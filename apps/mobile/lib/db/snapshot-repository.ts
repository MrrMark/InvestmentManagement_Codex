...
import type { MobileSqlDatabase } from './sqlite-types';
...
export async function listAccountsFromDb(db: MobileSqlDatabase): Promise<MobileAccount[]> {
...
export async function listSnapshotsFromDb(db: MobileSqlDatabase): Promise<MobileSnapshot[]> {
...
export async function getSnapshotByIdFromDb(
  db: MobileSqlDatabase,
  id: string,
): Promise<MobileSnapshot | null> {
...
export async function createSnapshotInDb(db: MobileSqlDatabase, input: CreateSnapshotInput) {
...
export async function importSnapshotsIntoDb(
  db: MobileSqlDatabase,
  inputs: readonly CreateSnapshotInput[],
) {
  const now = new Date().toISOString();
  let createdCount = 0;
...
export async function updateSnapshotInDb(db: MobileSqlDatabase, input: UpdateSnapshotInput) {
  const now = new Date().toISOString();
...
export async function deleteSnapshotFromDb(db: MobileSqlDatabase, id: string) {
