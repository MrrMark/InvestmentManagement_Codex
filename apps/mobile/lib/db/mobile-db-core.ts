...
export const mobileDatabaseVersion = 1;

export async function prepareMobileDatabase(db: MobileSqlDatabase) {
  await db.execAsync('PRAGMA foreign_keys = ON;');
  await migrateDbIfNeeded(db);
  await seedDbIfEmpty(db);
}

export async function migrateDbIfNeeded(db: MobileSqlDatabase) {
  const versionRow = await db.getFirstAsync<{ user_version: number }>('PRAGMA user_version');
  const currentVersion = versionRow?.user_version ?? 0;

  if (currentVersion >= mobileDatabaseVersion) {
    return;
  }
  ...
  await db.execAsync(`
    ...
    PRAGMA user_version = ${mobileDatabaseVersion};
  `);
}

export async function seedDbIfEmpty(db: MobileSqlDatabase) {
